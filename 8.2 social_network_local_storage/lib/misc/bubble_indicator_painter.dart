...
class BubbleIndicatorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path = Path();
    path.addArc(Rect.fromCircle(center: entry, radius: radius), 0.5 * pi,
        pi); //arco di sx
    path.addRect(Rect.fromLTRB(
        entry.dx, dy - radius, target.dx, dy + radius)); // quadrato al centro
    path.addArc(Rect.fromCircle(center: target, radius: radius), 1.5 * pi,
        pi); // arco di dx
